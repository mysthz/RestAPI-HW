...
import { useUserSubscriptions, useUser } from '../hooks/useUsers';
...
function UserSubscriptions() {
  ...
  return (
    <div>
      ...
      {isError && <ErrorMessage message="Ошибка загрузки подписок" />}
      {data && (
        <div className="flex flex-col gap-4">
          {data.subscribes.length === 0 ? (
            <p className="text-center text-muted-foreground">Подписки не найдены</p>
          ) : (
            data.subscribes.map((sub) => (
              <SubscriptionCard key={sub.id} authorId={sub.authorId} />
            ))
          )}
        </div>
      )}
