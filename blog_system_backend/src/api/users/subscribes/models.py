...
class Subscribe(Base):
    ...
    authorId: Mapped[int] = mapped_column(
        ForeignKey("users.id", onupdate="CASCADE", name="fk_subscribes_author"), index=True, nullable=False
    )
    subscriberId: Mapped[int] = mapped_column(
        ForeignKey("users.id", onupdate="CASCADE", name="fk_subscribes_subscriber"), index=True, nullable=False
    )
